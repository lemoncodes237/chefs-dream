...
Hence, to ensure correctness, it is vital that this specific order is obeyed in all branches of the `cook` function. If we check for ingredients in a wrong order, we may end up jumping to a wrong branch and not reach the correct recipe. The current order of ingredients is listed below; any new ingredients should preferrably be added to the end of the list so that adding new ingredients do not cause old branches to have the wrong order. However, it should be noted that for the sake of making sandwiches, condiments such as ketchpu and mustard should try to be maintained at the bottom of the list so that they can be added without invalidating recipes.
